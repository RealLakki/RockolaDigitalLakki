...
export function NowPlayingMini({ nowPlaying }: Props) {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(ref.current,
      { y: 10, opacity: 0, scale: 0.96, boxShadow: '0 0 0px rgba(240,90,26,0)' },
      { y: 0,  opacity: 1, scale: 1,    boxShadow: '0 0 28px rgba(240,90,26,0.5)',
        duration: 0.55, ease: 'back.out(1.6)' }
    );

    // Glow respiratorio continuo
    glowRef.current = gsap.to(ref.current, {
      boxShadow: '0 0 48px rgba(240,90,26,0.85), 0 0 90px rgba(240,90,26,0.25)',
